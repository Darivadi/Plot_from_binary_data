...
Poten_us = numpy.asarray(Poten_bin)
...
for i in range(0,20,int(SliceSize)):
    ...
    Poten_filter = Poten_us[z_filter]
    ...
    ***** Sorting by index *****                                                                       
    ...
    print "Plotting"
    cmap='RdBu_r' #good
    norm = matplotlib.colors.Normalize(vmin=Poten_s.min(),vmax=Poten_s.max())
    
    
    pyplot.close('all')
    pyplot.figure(figsize=(width,height))
    pyplot.scatter(x_s, y_s, s=7, c=Poten_s, cmap=cmap, edgecolors='none', norm=norm, marker='s') #This works!
    ...
    pyplot.title(r"$\Phi(\vec{r})$ for $%d \leq z < %d$"%(i,k) )
    pyplot.colorbar()
    pyplot.savefig("Potential_full_z_from_%dto%d.png"%(i,k) )
    #pyplot.show()
