...
for i in range(0,20,int(SliceSize)):
    ...
    ***** Sorting by index *****                                                                       
    ...
    norm = matplotlib.colors.Normalize(vmin=PotDot_s.min(),vmax=PotDot_s.max())
    
    
    pyplot.close('all')
    pyplot.figure(figsize=(width,height))
    pyplot.scatter(x_s, y_s, s=7, c=PotDot_s, cmap=cmap, edgecolors='none',
    ...
    pyplot.colorbar()
    pyplot.savefig("PotDot_app2_z_from_%dto%d.png"%(i,k) )
    #pyplot.show()
